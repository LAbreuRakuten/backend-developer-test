...
CREATE UNIQUE NONCLUSTERED INDEX [IX_User_RG] ON [dbo].[User]
(
	[RG] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, SORT_IN_TEMPDB = OFF, IGNORE_DUP_KEY = OFF, DROP_EXISTING = OFF, ONLINE = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
GO
...
BEGIN
	...
	SELECT * 
		FROM [User] 
		WHERE	(@Id = 0 OR Id = @Id) AND (@DocumentId IS NULL OR DocumentId = @DocumentId) AND (@Email IS NULL OR Email = @Email) OR (@RG IS NULL OR RG = @RG)
...
AS
BEGIN
UPDATE [dbo].[Order] SET CurrentStatus = @status WHERE Id = @id
END

CREATE PROCEDURE [dbo].[GetOrderNotIntegrated]
AS
BEGIN
SELECT * FROM [dbo].[Order] WHERE [Integrated] = 0
END
